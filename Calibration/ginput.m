function [out1,out2,out3] = ginput(varargin)
% Modified Version with the ability to control the cursor color
% The color is controlled by name-value pair: 'Color',[R,G,B] 
% where R,G,B are values in range [0,1]
% Author: Hanpeng
% UNC-CH
...
[how_many,color]=parseInputs(varargin{:});
b=[];
...
function [how_many,color]=parseInputs(varargin)
parser = inputParser;
parser.addRequired('number',@isPositiveScalarIntegerNumber);
parser.addParameter('color',[1,0,0],@Checkcolor);

parser.parse(varargin{:});
how_many = parser.Results.number;
...
function valid = Checkcolor(color)
validateattributes(color,{'numeric'},{'numel',3,'>=',0,'<=',1},mfilename,'color');
valid = true;
end
